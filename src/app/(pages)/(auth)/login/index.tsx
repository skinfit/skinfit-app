...
export default function LoginPage() {    
    ...
    const { user, error, signInWithEmail, signInWithProvider, signOut } = useAuth();
        ...
        return (
                <div className='flex flex-col space-y-4'>
                    <form className='flex flex-col space-y-4' onSubmit={handleSubmitWithEmail}>
                        ...
                        <button className="rounded px-4 py-4 font-semibold focus:outline-none bg-red-500 text-white focus:border-red-700 autofill:bg-transparent" type="submit">ログイン</button>
                    </form>
                    { error && <p className="text-red-400 text-sm text-center">{error.message}</p> }
                    <p className="text-gray-500 text-md text-center">
                        新規登録は<Link href="/signup" className="text-red-400 cursor-pointer mx-2 font-semibold">こちら</Link>
                    </p>
                    <p className="text-gray-500 text-md text-center">または</p>
                    <button className="flex flex-row items-center justify-left rounded border border-red-500 px-8 py-4 focus:outline-none bg-transparent space-x-4 hover:bg-red-100 focus:border-red-700 autofill:bg-transparent" type="button" onClick={async () => await signInWithProvider("google")}>
                        <Logo name="google" />
                        <p className='text-red-500'>Googleでサインイン</p>
                    </button>
                    <button className="flex flex-row items-center justify-left rounded border border-red-500 px-8 py-4 focus:outline-none bg-transparent space-x-4 hover:bg-red-100 focus:border-red-700 autofill:bg-transparent" type="button" onClick={async () => await signInWithProvider("apple")}>
                        <Logo name="apple" />
